...
1. Work in a **normal claude.ai chat**. You will build one **HTML artifact** (the dashboard) and, for each skill it needs, either confirm that skill already exists or **create it** — see Section 4, Step D. Skills are the thing that do the real work (reading Calendar/Gmail, parsing a PDF/ZIP, writing a fragment); the artifact itself is just a branded launcher + results viewer.
...
- **Does a "-cc" skill for this already exist** (ask, or check the environment's skill list)? If yes, reuse it. If not, one must be created in the shape defined by Appendix B (the dashboard skill template) before that card will work — Step D covers the check-and-package flow.
...
For each card, check the environment's available skills for a matching "-cc" skill (or ask the user, or search — a skill's own `description` field is what makes it discoverable). Sort each card's skill into one of three buckets:
- **Already exists as a "-cc" skill** → reuse it untouched, just point the card at it (Step C).
- **Exists only as an ordinary chat skill** (no dashboard-delivery version) → Step D covers making a "-cc" copy.
- **Doesn't exist at all** → Step D covers building one from scratch.
...
### Step D — Make sure every card's skill actually exists
A card whose prompt names a skill that was never built or saved will copy a prompt that goes nowhere. For each card: if a matching dashboard-delivery ("-cc") skill exists, reuse it and move on. If it doesn't, the skill must be created (or an existing chat skill adapted) so that it conforms to the **dashboard skill template in Appendix B** — that appendix defines the exact shape a skill needs (its output fragment contract and its cc-results delivery step) to integrate with this board. If the user provided a custom brand, **rewrite every reference hex in each skill's fragment-rendering step with the user's hexes** — this is the other half of the branding pass; the artifact's `:root` does nothing for skill output. Once a skill exists in that shape (and in the user's palette), package it (`python -m scripts.package_skill <skill-folder> <output-dir>`, run per the skill-creator skill's instructions), present the `.skill` file, and have the user click **Save skill** — the card's copied prompt will not work until the skill is saved to their profile.
...
- Brand palette hard-coded (a fragment can't see the shell's CSS variables) — use the hexes from Section 5. Keep green `#4f9d7c` for positive/pass and red `#b3433f` for negative/fail regardless of brand.
